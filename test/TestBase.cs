...
namespace test
{
	...
	public class TestBase
	{
		...
		public void Test64_Decode_1()
		{
			...
			var enu = TuckBytes.Decode(s,CodecBase64.Self);
			string test = String.Concat(enu);
			Assert.AreEqual(OrigText.Length,test.Length);
			for(int c=0; c<OrigText.Length; c++) {
				Trace.WriteLine(c+" ["+OrigText[c]+","+test[c]+"]");
				Assert.AreEqual(OrigText[c],test[c]);
			}
		}
		...
		[TestMethod]
		public void Test85_Decode_1()
		{
			var s = new MemoryStream(Encoding.ASCII.GetBytes(B85Text));
			var enu = TuckBytes.Decode(s,CodecBase85.Self);
			string test = String.Concat(enu);
			Assert.AreEqual(OrigText.Length,test.Length);
			for(int c=0; c<OrigText.Length; c++) {
				Trace.WriteLine(c+" ["+OrigText[c]+","+test[c]+"]");
